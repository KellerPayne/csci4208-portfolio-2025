# <Capstone Final Project Full Demo> — <capstone-09>

**Team repo:** [Keller's Capstone Repo](https://github.com/KellerPayne/csci4208-capstone-project-repo)
**Project (v2) board:** <URL> [Sprint 4 Project Board](https://github.com/users/KellerPayne/projects/10)  
**Live demo / recording (if applicable):** <URL>

## Deliverables for this phase
- Demo of complete application

## Summary (what we produced)
- Fully completed work on project
- Checked that all test came back with no errors one last time

## Evidence / Artifacts
- [Capstone Project Repo README.md File](https://github.com/KellerPayne/csci4208-capstone-project-repo/blob/main/README.md)

## Notes & Risks
- Right now, only runs on localhost